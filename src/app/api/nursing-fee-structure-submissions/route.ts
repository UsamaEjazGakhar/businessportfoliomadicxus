...
const nursingFeeStructureSubmissionSchema = z.object({
...
export async function GET(req: NextRequest) {
  try {
    ...
    const forms = await prisma.nursingFeeStructureSubmission.findMany({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validatedData = nursingFeeStructureSubmissionSchema.parse(body);

    const form = await prisma.nursingFeeStructureSubmission.create({
      data: validatedData,
    });
