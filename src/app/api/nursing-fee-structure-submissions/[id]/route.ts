...
const nursingFeeStructureSubmissionUpdateSchema = z.object({
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const form = await prisma.nursingFeeStructureSubmission.findUnique({
      where: { id },
    });
    if (!form) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validatedData = nursingFeeStructureSubmissionUpdateSchema.parse(body);

    const form = await prisma.nursingFeeStructureSubmission.update({
      where: { id },
      data: validatedData,
    });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.nursingFeeStructureSubmission.delete({
      where: { id },
    });
