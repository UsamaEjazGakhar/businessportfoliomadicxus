...
const paramedicalFeeMLTUpdateSchema = z.object({
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const feeStructure = await prisma.paramedicalFeeStructureMLTTemplate.findUnique({
      where: { id },
    });
    if (!feeStructure) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validatedData = paramedicalFeeMLTUpdateSchema.parse(body);

    const feeStructure = await prisma.paramedicalFeeStructureMLTTemplate.update({
      where: { id },
      data: validatedData,
    });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.paramedicalFeeStructureMLTTemplate.delete({
      where: { id },
    });
