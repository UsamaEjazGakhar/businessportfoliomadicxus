...
const paramedicalFeeMLTSchema = z.object({
...
export async function GET(req: NextRequest) {
  try {
    const feeStructures = await prisma.paramedicalFeeStructureMLTTemplate.findMany();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validatedData = paramedicalFeeMLTSchema.parse(body);

    const feeStructure = await prisma.paramedicalFeeStructureMLTTemplate.create({
      data: validatedData,
    });
