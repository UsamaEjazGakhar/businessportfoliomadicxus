...
const paramedicalFeeSchema = z.object({
...
export async function GET(req: NextRequest) {
  try {
    const feeStructures = await prisma.paramedicalFeeStructureLabTemplate.findMany();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validatedData = paramedicalFeeSchema.parse(body);

    const feeStructure = await prisma.paramedicalFeeStructureLabTemplate.create({
      data: validatedData,
    });
