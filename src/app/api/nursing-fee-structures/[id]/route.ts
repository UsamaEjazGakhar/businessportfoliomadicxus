...
const nursingFeeUpdateSchema = z.object({
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const feeStructure = await prisma.nursingFeeStructureTemplate.findUnique({
      where: { id },
    });
    if (!feeStructure) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validatedData = nursingFeeUpdateSchema.parse(body);

    const feeStructure = await prisma.nursingFeeStructureTemplate.update({
      where: { id },
      data: validatedData,
    });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.nursingFeeStructureTemplate.delete({
      where: { id },
    });
