...
const paramedicalFeeStructureMLTSubmissionUpdateSchema = z.object({
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const form = await prisma.paramedicalFeeStructureMLTSubmission.findUnique({
      where: { id },
    });
    if (!form) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validatedData = paramedicalFeeStructureMLTSubmissionUpdateSchema.parse(body);

    const form = await prisma.paramedicalFeeStructureMLTSubmission.update({
      where: { id },
      data: validatedData,
    });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.paramedicalFeeStructureMLTSubmission.delete({
      where: { id },
    });
