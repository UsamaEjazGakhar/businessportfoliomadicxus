...
interface NursingFeeStructureType {
  id: string;
...
interface NursingFeeStructureProps {
  onGoBack?: () => void;
}

export default function NursingFeeStructure({ onGoBack }: NursingFeeStructureProps) {
  const [feeStructure, setFeeStructure] = useState<NursingFeeStructureType | null>(null);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`nursing-fee-structure-${formData.applicantName || 'unnamed'}.pdf`);
    } catch (error) {
  ...
  useEffect(() => {
    async function fetchFeeStructure() {
      try {
        const res = await fetch('/api/nursing-fee-structures');
        ...
        if (json.success && json.data.length > 0) {
          ...
          setFeeStructure({
            id: 'default',
            part1AdmissionFee: '75000.00',
            part1MonthlyFee: '10000.00',
            part1TotalFee: '195000.00',
            part2MonthlyFee: '10000.00',
            part2TotalFee: '120000.00',
            note: 'All Fee are "Non Refundable"',
            examFeeNote: 'Examination Fee will be paid according to Nursing Council of Pakistan',
            admissionCriteria: 'Matric with Science\n45% iverall',
            ageLimit: '35 years',
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%', height: '100%' }}>
          <div style={{ textAlign: 'center', marginBottom: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 'bold' }}>Care Institute of Health Sciences</div>
                <div style={{ fontSize: '11px' }}>Satellite Town, Rawalpindi.</div>
                <div style={{ fontSize: '11px' }}>Conact No. 0333-1165573</div>
              </div>
            </div>
            <div style={{ fontSize: '12px', fontWeight: 'bold' }}>Admission Fee Structure</div>
            <div style={{ fontSize: '11px' }}>(Nursing Program (02 Years Program))</div>
            <div style={{ fontSize: '11px' }}>(Part 01)</div>
          </div>
            ...
            <tbody>
              <tr>
                <td rowSpan={3} style={{ border: '1px solid #555', padding: '5px 8px', verticalAlign: 'middle', lineHeight: '1.8' }}>
                  L.H.V.<br />Certified Nurse<br />Assistant (CAN)<br />C.M.W
                </td>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', borderBottom: '1px solid #555' }}>
            ...
            <tbody>
              <tr>
                <td rowSpan={2} style={{ border: '1px solid #555', padding: '5px 8px', verticalAlign: 'middle', fontWeight: 'bold' }}>Part 02</td>
          ...
          <div style={{ padding: '8px', borderBottom: '1px solid #555', fontSize: '11px', textAlign: 'center' }}>
            {feeStructure.examFeeNote}
          </div>
