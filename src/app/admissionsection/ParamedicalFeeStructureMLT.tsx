...
interface ParamedicalFeeStructureMLTType {
  id: string;
...
interface ParamedicalFeeStructureMLTProps {
  onGoBack?: () => void;
}

export default function ParamedicalFeeStructureMLT({ onGoBack }: ParamedicalFeeStructureMLTProps) {
  const [feeStructure, setFeeStructure] = useState<ParamedicalFeeStructureMLTType | null>(null);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`paramedical-mlt-fee-structure-${formData.applicantName || 'unnamed'}.pdf`);
    } catch (error) {
  ...
  useEffect(() => {
    async function fetchFeeStructure() {
      try {
        const res = await fetch('/api/paramedical-fee-structure-mlt');
            ...
            <tbody>
              <tr>
                <td rowSpan={3} style={{ border: '1px solid #555', padding: '5px 8px', verticalAlign: 'middle', lineHeight: '1.8' }}>
                  MLT<br />OTT<br />Rediology<br />Dispenser<br />Optometry<br />Physiotherapy<br />Dental Hygenic
                </td>
